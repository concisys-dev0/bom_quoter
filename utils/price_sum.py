...
# SMT terminals
def price_labor_SMT(path, order, df):
    ...
    for i in range(l):
        if mountType_list[i] == "SMT":
            smt_terminals = terminals_list[i]*qty_list[i] # number of terminals per board for 1 item
...
# TH terminals
def price_labor_TH(path, order, df):
    L = len(df.axes[0]) # length of df
    qty_list = []
    mountType_list = []
    terminals_list = []
...
# MEC terminals
def price_labor_MEC(path, order, df):
    L = len(df.axes[0]) # length of df
    qty_list = []
    mountType_list = []
    terminals_list = []
...
def empty_warning(df):
    ...
    # df.isnull(df['Terminations']) # .iloc[0]
...
def df_summary(path, df):
    ...
    sum_oext[0] = price_sum_ext(path, df)
    if order_list[1] != 0:
        sum_oext[1] = price_sum_Q1ext(path, df)
    else:
        sum_oext[1] = 0
    if order_list[2] != 0:
        sum_oext[2] = price_sum_Q2ext(path, df)
    else:
        sum_oext[2] = 0
    if order_list[3] != 0:
        sum_oext[3] = price_sum_Q3ext(path, df)
    else:
        sum_oext[3] = 0
    if order_list[4] != 0:
        sum_oext[4] = price_sum_Q4ext(path, df)
    else:
        sum_oext[4] = 0
    if order_list[5] != 0:
        sum_oext[5] = price_sum_Q5ext(path, df)
    else:
        sum_oext[5] = 0
...
def save_summary(path, df):
    df = df_summary(path, df)
    print(df)
