...
def wrap_exception_in(exc_type, catch=Exception):
    ...
    exponential_backoff = retrying.retry(
        retry_on_exception=lambda exc: isinstance(exc, RequestException), # Retry when RequestException is raised.
        wait_exponential_multiplier=100, # wait 2^x * 100 ms between each retry,
        wait_exponential_max=10000, # wait up to 10s between each retry,
        stop_max_delay=20000) # and stop retrying after 20 total seconds.
